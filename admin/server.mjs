...
Plus body rule: Do not put real homepage URLs, official URL text, external links, or anchor tags in the A/plus article body. Use plain Korean phrases such as "공식 안내 화면" instead. Info article body should also avoid bare URL text because official navigation is handled by system CTA buttons.
...
`;
  ...
  parsed.plus.html = removeExternalUrlsFromPlusHtml(normalizeArticleHtml(parsed.plus.html));
  parsed.info.html = removeExternalUrlsFromPlusHtml(normalizeArticleHtml(parsed.info.html));
  return parsed;
}
...
function buildPaxnetHtml(draft, plusUrl, linkText) {
  ...
  const stepHtml = steps
    .map((step, index) => {
      ...
      return `<div style="${style}"><strong>${index + 1}.</strong> ${escapeHtml(step)}</div>`;
    })
    .join("");

  return `<p><br></p><div style="padding:20px 0; margin:16px 0; background:#ffffff; font-family:'Apple SD Gothic Neo','Pretendard','Noto Sans KR',sans-serif; line-height:1.7; color:#111;"><span style="display:inline-block; background:#e9faf5; color:#008b72; font-size:13px; font-weight:700; padding:7px 12px; border-radius:999px; margin-bottom:14px;">${escapeHtml(draft.label)}</span><h2 style="font-size:24px; margin:0 0 12px; line-height:1.4; color:#111;">${escapeHtml(draft.title)}</h2><p style="margin:0 0 12px; font-size:16px;">${escapeHtml(draft.intro)}</p><a href="${escapeHtml(plusUrl)}" target="_blank" style="display:block; margin-top:20px; background:#e53935; color:#fff; padding:18px 20px; font-size:18px; font-weight:700; border-radius:50px; text-align:center; text-decoration:none; box-shadow:0 6px 12px rgba(229,57,53,0.25);">${escapeHtml(linkText)}</a><br><p style="margin:0 0 12px; font-size:16px;">${escapeHtml(draft.paragraph1)}</p><p style="margin:0 0 12px; font-size:16px;">${escapeHtml(draft.paragraph2)}</p><div style="border-top:1px solid #e5e7eb; margin:20px 0; padding-top:16px;"><p style="margin:0 0 10px; font-size:16px; font-weight:700;">${escapeHtml(draft.boxTitle)}</p><p style="margin:0 0 10px; font-size:16px;">${escapeHtml(draft.boxParagraph1)}</p><p style="margin:0; font-size:16px;">${escapeHtml(draft.boxParagraph2)}</p></div><p style="margin:0 0 12px; font-size:16px;">${escapeHtml(draft.afterBoxParagraph1)}</p><p style="margin:0 0 12px; font-size:16px;">${escapeHtml(draft.afterBoxParagraph2)}</p><div style="background:#f8f9fb; border-radius:12px; padding:15px; margin:18px 0 0;">${stepHtml}</div></div>`;
}
...
async function makePaxnetDraft(input) {
  if (!hasOpenAiKey()) {
    throw new Error("팍스넷 발행문 생성에는 OPENAI_API_KEY가 필요합니다.");
  }
...
`;
  ...
  const response = await fetch("https://api.openai.com/v1/responses", {
    ...
    body: JSON.stringify({
      ...
      text: {
        format: {
          type: "json_schema",
          name: "liferoom_paxnet_draft",
          strict: true,
          schema: paxnetDraftSchema()
        }
      }
    })
  });
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    const message = data?.error?.message || "팍스넷 발행문 생성에 실패했습니다.";
    throw new Error(message);
  }

  const parsed = JSON.parse(outputTextFromResponse(data));
  if (!parsed.title || !parsed.intro || !Array.isArray(parsed.steps)) throw new Error("팍스넷 발행문 응답 형식이 올바르지 않습니다.");
  ...
  return {
    ...
    plusUrl,
    title: parsed.title,
    html: buildPaxnetHtml(parsed, plusUrl, linkText),
...
작성 규칙:
- 한국어로 자연스럽게 씁니다. 관공서 안내문이나 AI 요약문처럼 딱딱하게 쓰지 않습니다.
...
- 문장 길이를 일부러 섞습니다. 짧은 문장과 긴 문장을 함께 쓰고, 모든 문장을 같은 패턴으로 끝내지 않습니다.
- "본 글에서는", "아래에서 자세히", "핵심 정보를 정리했습니다", "놓치지 마세요" 같은 AI/광고 느낌 표현은 피합니다.
- 같은 단어를 반복하지 말고, "확인", "조회", "살펴보기", "체크"처럼 자연스럽게 바꿔 씁니다.
- 소제목은 기계적인 명사형보다 "전주공항버스 예매 전에 먼저 볼 것"처럼 검색 키워드와 생활 문장이 섞이게 씁니다.
...
- B 블로그 내용을 그대로 복사하지 말고, 네이버 검색자가 궁금해할 순서로 다시 설명합니다.
- 정보는 충분히 주되, 마지막에는 "제가 따로 보기 쉽게 정리해둔 글"을 확인하는 흐름으로 A 블로그 링크를 안내합니다.
...
async function makeDraft(input) {
  ...
  const plusHtml = removeExternalUrlsFromPlusHtml(aiDraft?.plus?.html || buildPlusHtml({ keyword, button1, button2, contentType }));
  const infoHtml = removeExternalUrlsFromPlusHtml(aiDraft?.info?.html || buildInfoHtml({ keyword, officialUrl, button1, button2, contentType }));
...
async function handleApi(req, res, pathname) {
  try {
    ...
    if (req.method === "POST" && pathname === "/api/paxnet-draft") {
      return sendJson(res, 200, await makePaxnetDraft(await readBody(req)));
    }
